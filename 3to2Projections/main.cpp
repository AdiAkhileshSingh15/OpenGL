...
namespace Geometry
{
    ...
    void Main()
    {
        ...
        if (choice == 1)
        {
            ...
            mat.val = {{d1 - x * n1, -x * n2, -x * n3, x * d0}, {-y * n1, d1 - y * n2, -y * n3, y * d0}, {-z * n1, -z * n2, d1 - z * n3, z * d0}, {0, 0, 0, float(d1)}};
        }
        else
        {
            assert(choice == 2);
            // transformation matrix for parallel projection
            mat.val = {{d + x * n1, x * n2, x * n3, -d0 * x}, {y * n1, d + y * n2, y * n3, -d0 * y}, {z * n1, z * n2, d + z * n3, -d0 * z}, {n1, n2, n3, -float(d1)}};
        }
